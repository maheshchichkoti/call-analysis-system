...
class AlertWorker:
    ...
    3. Update record with sent status
    """
    ...
    def process_batch(self) -> int:
        ...
        for record in pending:
            try:
                ...
                logger.error(f"Failed to send alert for {record['id']}: {e}")
    ...
    def _send_alert(self, record: Dict[str, Any]):
        """Send alert email for a single record."""
        record_id = record["id"]

        logger.info(f"Sending alert for {record_id}")

        # Parse warning reasons
        warning_reasons = []
        if record.get("warning_reasons_json"):
            try:
                warning_reasons = json.loads(record["warning_reasons_json"])
        ...
        call_data = {
            ...
            "transcript_text": record.get("transcript_text", "")[:3000],  # Limit size
        }
